...
exports.getUsers = (req, res, next) => {
    return res.status(httpStatus["201_MESSAGE"]).json({
        message: "",
    });
};

exports.getUser = (req, res, next) => {
    return res.status(httpStatus["200_MESSAGE"]).json({
        message: "",
    });
};
exports.addUser = async(req, res, next) => {
    ...
    await User.findOne({ email: req.body.email }).then((data) => {
        ...
        if (data) {
            return res.status(httpStatus["404_CLASS"]).json({
        ...
        bcrypt.hash(req.body.password, 10).then((hashedPass) => {
            User.count()
                .then((numOfDocuments) => {
                    ...
                    user.save().then((e) => {
                        return res.status(httpStatus["200_CLASS"]).json({
...
exports.loginUser = (req, res, next) => {
    ...
    User.findOne({ email: req.body.email }).then((user) => {
        ...
        bcrypt.compare(req.body.password, user.password).then((validPass) => {
            ...
            const token = jwt.sign({ _id: userData._id, email: userData.email },
                constant.JWT_SECRET, {
                    expiresIn: "7d",
                }
            );
